...
const byte NbrRows = 4; 
const byte NbrCols = 4;
...
void keypad_initalize(){
    ...
    for(int i = 0; i < NbrRows; i++){
        gpio_set_direction(NbrRows[i], GPIO_MODE_INPUT);
        gpio_set_pull_mode(NbrRows[i], GPIO_PULLUP_ONLY)
    }
}
void keypad_output_set(keypad_output_level_t col0, keypad_output_level_t col1, keypad_cols_t col2, keypad_cols_t col3){
    gpio_set_level(NbrCols[0], col0);
    gpio_set_level(NbrCols[1], col1);
    gpio_set_level(NbrCols[2], col2);
    gpio_set_level(NbrCols[3], col3);
}

char keypad_detect(keypad_cols_t col)
    if (gpio_get_level(NbrRows[0]) == 0){
        return keypad[0][col];
    else if (gpio_get_level(NbrRows[1]) == 0)
        return keypad[1][col];
    else if (gpio_get_level(NbrRows[2]) == 0)
        return keypad[2][col];
    else if (gpio_get_level(NbrRows[3]) == 0)
        return keypad[3][col];
    return 255;
}
